...
def generate_image(prompt: str, size: str = None, width: int = None, height: int = None, output_dir: str = "images") -> Optional[str]:
    ...
    Args:
        prompt: 图片生成提示词
        size: 图片尺寸预设 (可选: "cover"/封面图, "content"/文中配图, "banner"/横幅)
        width: 自定义宽度 (像素)
        height: 自定义高度 (像素)
        output_dir: 图片保存目录
    ...
    尺寸规则：
        - cover (封面图): 900x383 横向，适合公众号封面
        - content (文中配图): 900x600 横向，适合文中插图
        - banner (横幅): 900x300 横向，适合头图/分割图
        - 自定义: 指定width和height参数
        - 默认: 900x600
    """
    client = init_client()
    ...
    if size == "cover" or size == "封面":
        final_size = "900x383"  # 公众号封面标准比例 2.35:1
    elif size == "banner" or size == "横幅":
        final_size = "900x300"  # 横幅 3:1
    elif size == "content" or size == "配图":
        final_size = "900x600"  # 文中配图 3:2
    elif width and height:
        final_size = f"{width}x{height}"
    else:
        final_size = "900x600"  # 默认横向尺寸
...
def main():
    ...
    parser.add_argument("--size", help="图片类型: cover/封面图, content/文中配图, banner/横幅")
    parser.add_argument("--width", type=int, help="自定义宽度（像素）")
    parser.add_argument("--height", type=int, help="自定义高度（像素）")
    ...
    if not args.prompt and not args.optimize:
        ...
        print("=" * 60)
        print("\n用法示例:")
        print('  python generate_image.py -p "科技感的公众号封面图" --size cover')
        print('  python generate_image.py -p "文中插图" --size content')
        print('  python generate_image.py -p "横幅图片" --size banner')
        print('  python generate_image.py -p "自定义尺寸" --width 800 --height 600')
        print('  python generate_image.py -p "未来城市" -O  # 使用AI优化提示词')
        print("\n参数说明:")
        print("  -p, --prompt    图片描述（英文或中文）")
        print("  -s, --style     风格偏好（可选）")
        print("  --size          图片类型: cover(900x383), content(900x600), banner(900x300)")
        print("  --width         自定义宽度（像素）")
        print("  --height        自定义高度（像素）")
        print("  -o, --output    输出目录 (默认: images)")
        print("  -O, --optimize  使用AI优化提示词")
        print("\n注意:")
        print("  需要在.env文件中设置ARK_API_KEY环境变量")
        print("  默认尺寸为 900x600（横向，适合公众号配图）")
        print("=" * 60)
        return
    ...
    size_info = args.size or f"{args.width}x{args.height}" or "900x600"
    print(f"尺寸: {size_info}")

    image_path = generate_image(final_prompt, args.size, args.width, args.height, args.output)
